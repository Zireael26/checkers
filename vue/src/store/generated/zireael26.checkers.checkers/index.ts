import { Client, registry, MissingWalletError } from 'Zireael26-checkers-client-ts'

import { Params } from "Zireael26-checkers-client-ts/zireael26.checkers.checkers/types"
import { StoredGame } from "Zireael26-checkers-client-ts/zireael26.checkers.checkers/types"
import { SystemInfo } from "Zireael26-checkers-client-ts/zireael26.checkers.checkers/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: zireael26.checkers.checkers initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.Zireael26CheckersCheckers.query.queryParams()).data
		...
		async QuerySystemInfo({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.Zireael26CheckersCheckers.query.querySystemInfo()).data
		...
		async QueryStoredGame({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.Zireael26CheckersCheckers.query.queryStoredGame( key.index)).data
		...
		async QueryStoredGameAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.Zireael26CheckersCheckers.query.queryStoredGameAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.Zireael26CheckersCheckers.query.queryStoredGameAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgCreateGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.Zireael26CheckersCheckers.tx.sendMsgCreateGame({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgRejectGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.Zireael26CheckersCheckers.tx.sendMsgRejectGame({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgPlayMove({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.Zireael26CheckersCheckers.tx.sendMsgPlayMove({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgCreateGame({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.Zireael26CheckersCheckers.tx.msgCreateGame({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateGame:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgRejectGame({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.Zireael26CheckersCheckers.tx.msgRejectGame({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgRejectGame:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgPlayMove({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.Zireael26CheckersCheckers.tx.msgPlayMove({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgPlayMove:Init Could not initialize signing client. Wallet is required.')
